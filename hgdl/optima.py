...
class optima:
    ...
    def fill_in_optima_list(self,res):
        ...
        for i in range(len(clean_x)):
            if any(clean_g[i] > 1e-5): classifier.append("degenerate")
            ...
            elif abs(np.max(np.sign(clean_eig[i] )) - np.min(np.sign(clean_eig[i] ))) == 2: classifier.append("saddle point")
            else: classifier.append("ERROR")


        new_optima_list = []
        for i in range(len(clean_x)):
            new_optima_list.append(self.make_optima_list_entry(clean_x[i],clean_f[i],classifier[i],clean_eig[i],clean_g[i],np.linalg.norm(clean_g[i]),clean_radii[i]))
    ...
    def get_deflation_points(self,n):
        try:
            defl_list = [entry for entry in self.list if entry["classifier"] == "maximum" or entry["classifier"]  == "minimum" or entry["classifier"]  == "saddle point"]
